...
const Hero = () => (
  <section className="relative min-h-screen flex items-center pt-20 overflow-hidden">
    ...
    <div className="absolute inset-0 z-0 flex">
      {/* Left side (empty for content space) */}
      <div className="w-1/2 hidden md:block bg-background" />

      {/* Right side image */}
      <div
        className="w-full md:w-1/2 h-full bg-no-repeat bg-contain bg-right"
        style={{ backgroundImage: `url(${heroBg})` }}
      />
    </div>

    {/* Grid overlay (optional - kept light) */}
    ...
    <div className="max-w-7xl mx-auto px-6 relative z-10 py-20 w-full">
      ...
      >
        ...
        <h1 className="font-display text-4xl sm:text-6xl lg:text-7xl font-bold text-brand-navy leading-[0.9] mb-6 tracking-tighter">
          Reliable <span className="text-brand-orange">Workforce.</span>
          <br />
          Strong <span className="text-brand-orange">Infra.</span>
        </h1>
        ...
        <div className="flex flex-wrap gap-4">
          <a
            href="#contact"
            className="bg-brand-orange text-white px-8 py-4 font-bold flex items-center gap-2 group hover:bg-brand-teal transition-all duration-300 ease-brand"
          >
            GET A QUOTE
            <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
          </a>
          ...
          <a
            href="#services"
            className="border-2 border-brand-navy/30 text-brand-navy px-8 py-4 font-bold hover:border-brand-orange hover:text-brand-orange transition-all duration-300 ease-brand"
          >
            EXPLORE SERVICES
          </a>
        </div>
        ...
        <div className="mt-16 flex flex-wrap gap-12">
          {[
            ...
            >
              ...
              <p className="text-brand-navy/50 text-xs font-bold tracking-widest uppercase mt-1">
                {stat.label}
              </p>
            </motion.div>
          ))}
